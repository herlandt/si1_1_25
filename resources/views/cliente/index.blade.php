<x-app-layout>
    ...
    <div class="col-span-full xl:col-span-8 bg-white dark:bg-slate-900 shadow-lg rounded-sm">
        ...
        <header class="px-5 py-4 border-b border-slate-100 dark:border-slate-700">
            <h2 class="text-center font-semibold text-slate-800 dark:text-slate-100">Clientes</h2>
                ...
                <table class="table-auto w-full dark:text-slate-300">
                    ...
                    <tbody  id="searchTableBody" class="text-sm font-medium divide-y divide-slate-100 dark:divide-slate-700">
                        ...
                        @foreach ($clientes as $c)
                            <tr>
                                <td class="p-2">
                                    <div class="text-center text-emerald-500">{{ $c->id }}</div>
                                </td>
                                <td class="p-2">
                                    <div class="flex items-center">
                                        ...
                                        <div class="text-slate-800 dark:text-slate-100">{{ $c->nombre }}</div>
                                    </div>
                                </td>

                                <td class="p-2">
                                    <div class="text-center text-slate-800 dark:text-slate-100">{{ $c->celular }}
                                    </div>
                                </td>
                                ...
                                <td class="p-2">
                                    <div class="flex justify-center space-x-4">
                                        <!-- Botón de Editar -->
                                        <a title="EDITAR" href="{{ route('clientes.edit', $c->id) }}"
                                        ...
                                        <!-- Botón de Eliminar -->
                                        <a title="ELIMINAR" href="{{ route('clientes.destroy', $c->id) }}"
                                            onclick="event.preventDefault(); if (confirm('¿Estás seguro de eliminar este usuario?')) { document.getElementById('delete-form-{{ $c->id }}').submit(); }"
                                            class="rounded-lg p-2 text-white hover:scale-125 transition-transform delay-75">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        ...
                                        <form id="delete-form-{{ $c->id }}"
                                            action="{{ route('clientes.destroy', $c->id) }}" method="POST"
                                            style="display: none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
